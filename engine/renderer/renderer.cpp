...
std::unique_ptr<Renderer> Renderer::create(GpuContext& ctx,
...
Renderer::~Renderer()
{
    if (render_pass_)  { wgpuRenderPassEncoderRelease(render_pass_); render_pass_  = nullptr; }
...
bool Renderer::begin_frame()
{
    ...
    {
        std::fprintf(stderr, "[Renderer] Surface texture not ready (status=%d)\n",
...
{
    if (!render_pass_ || !pipeline_) { return; }
