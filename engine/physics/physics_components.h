...
enum class MotionType : uint8_t { Static, Dynamic, Kinematic };
enum class PhysicsLayer : uint8_t { Static = 0, Dynamic = 1, Trigger = 2 };

struct BoxShapeDesc { float half_x = 0.5f; float half_y = 0.5f; float half_z = 0.5f; };
struct SphereShapeDesc { float radius = 0.5f; };
struct CapsuleShapeDesc { float half_height = 0.5f; float radius = 0.25f; };
...
enum class ContactType : uint8_t { Begin, Persist, End };
